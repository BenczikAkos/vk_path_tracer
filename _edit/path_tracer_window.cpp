...
//
void PathTracerWindow::setup(const VkInstance& instance, const VkDevice& device, const VkPhysicalDevice& physicalDevice, uint32_t queueFamily)
{
...
void PathTracerWindow::createPostPipeline()
{
  ...
  pipelineGenerator.addShader(nvh::loadFile("shaders/passthrough.vert.glsl.spv", true, defaultSearchPaths, true), VK_SHADER_STAGE_VERTEX_BIT);
  pipelineGenerator.addShader(nvh::loadFile("shaders/post.frag.glsl.spv", true, defaultSearchPaths, true), VK_SHADER_STAGE_FRAGMENT_BIT);
...
void PathTracerWindow::createRtPipeline()
{
  ...
  // Raygen
  stage.module = nvvk::createShaderModule(m_device, nvh::loadFile("shaders/raytrace.rgen.glsl.spv", true, defaultSearchPaths, true));
  stage.stage     = VK_SHADER_STAGE_RAYGEN_BIT_KHR;
  stages[eRaygen] = stage;
  // Miss
  stage.module = nvvk::createShaderModule(m_device, nvh::loadFile("shaders/raytrace.rmiss.glsl.spv", true, defaultSearchPaths, true));
  stage.stage   = VK_SHADER_STAGE_MISS_BIT_KHR;
  stages[eMiss] = stage;
  // The second miss shader is invoked when a shadow ray misses the geometry. It simply indicates that no occlusion has been found
  stage.module =
      nvvk::createShaderModule(m_device, nvh::loadFile("shaders/raytraceShadow.rmiss.glsl.spv", true, defaultSearchPaths, true));
  stage.stage    = VK_SHADER_STAGE_MISS_BIT_KHR;
  stages[eMiss2] = stage;
  // Hit Group - Closest Hit
  stage.module = nvvk::createShaderModule(m_device, nvh::loadFile("shaders/raytrace.rchit.glsl.spv", true, defaultSearchPaths, true));
...
void PathTracerWindow::raytrace(const VkCommandBuffer& cmdBuf, const glm::vec4& clearColor)
{
  ...
  m_pcRay.frameCount++;  // Increment frame counter
